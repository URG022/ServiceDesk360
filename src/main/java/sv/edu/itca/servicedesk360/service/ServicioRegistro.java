...
import sv.edu.itca.servicedesk360.model.Administrador;
...
public class ServicioRegistro {
    ...
    public ServicioRegistro(BuscadorCuentas buscador, RegistradorCuentas registrador, ValidadorRegistro validador) {
        this.buscador = buscador;
        this.registrador = registrador;
        this.validador = validador;
    }

    public List<String> registrar(String nombre, String correo, String rol, String clave, String confirmacion) {
        List<String> errores = validador.validar(nombre, correo, rol, clave, confirmacion);
        ...
        if ("TECNICO".equalsIgnoreCase(rol)) {
            usuario = new Tecnico(id, nombre, correoNormalizado, "Soporte general", 1);
        } else if ("ADMINISTRADOR".equalsIgnoreCase(rol)) {
            usuario = new Administrador(id, nombre, correoNormalizado);
        } else {
            usuario = new Solicitante(id, nombre, correoNormalizado, "General", "");
        }

        CuentaUsuario cuenta = new CuentaUsuario(usuario, SeguridadClave.generarHash(clave));
